...
class ExpdirMonitor:
	...
	def load_run_config(self, print_info=False, dataset='imagenet'):
	...
	def run(self, train=True, is_test=True, valid_size=None, resume=False):
		init = self.load_init()
		run_config = self.load_run_config(print_info=True)
		...
		loss, acc1, acc5 = run_manager.validate(is_test=is_test, return_top5=True)
		if is_test:
			log = 'test_loss: %f\t test_acc1: %f\t test_acc5: %f' % (loss, acc1, acc5)
			run_manager.write_log(log, prefix='test')
			json.dump({'test_loss': '%f' % loss, 'test_acc1': '%f' % acc1, 'test_acc5': '%f' % acc5},
			          open(self.output_path, 'w'))
		else:
			log = 'valid_loss: %f\t valid_acc1: %f\t valid_acc5: %f' % (loss, acc1, acc5)
			run_manager.write_log(log, prefix='valid')
			json.dump({'valid_loss': '%f' % loss, 'valid_acc1': '%f' % acc1, 'valid_acc5': '%f' % acc5,
			           'valid_size': run_config.valid_size}, open(self.output_path, 'w'))
		return acc1
