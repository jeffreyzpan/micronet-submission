...
class RunConfig:
	...
	def __init__(self, n_epochs=300, init_lr=0.05, lr_schedule_type='cosine', lr_schedule_param=None,
	             dataset=None, train_batch_size=64, test_batch_size=100, valid_size=None,
	             drop_last=True, n_worker=20, distort_color=None,
	             opt_type='sgd', opt_param=None, weight_decay=5e-4, label_smoothing=0,
	             model_init='he_fout', init_div_groups=False,
	...
	@staticmethod
	def get_default_run_config(dataset):
...
class RunManager:
	...
	def print_net_info(self):
		...
		with open('%s/net_info.txt' % self.logs_path, 'w') as fout:
			fout.write(json.dumps(net_info) + '\n')
		...
		for epoch in range(self.start_epoch, self.run_config.n_epochs):
			...
			for i, (_input, target) in enumerate(data_loader):
				...
				if self.run_config.label_smoothing > 0:
					loss = cross_entropy_with_label_smoothing(output, target_var, self.run_config.label_smoothing)
				else:
				...
				if i % self.run_config.print_frequency == 0 or i + 1 == len(data_loader):
					batch_log = 'Train [{0}][{1}/{2}]\t' \
						...
						format(epoch + 1, i, len(data_loader) - 1,
					           batch_time=batch_time, data_time=data_time, losses=losses, top1=top1, top5=top5, lr=lr)
			...
			if (epoch + 1) % self.run_config.validation_frequency == 0:
				...
				val_log = 'Valid [{0}/{1}]\tloss {2:.3f}\ttop 1-acc {3:.3f} ({4:.3f})\ttop 5-acc {5:.3f}\t' \
					...
					format(epoch + 1, self.run_config.n_epochs, val_loss, val_acc, self.best_acc, val_acc5,
				           top1=top1, top5=top5)
